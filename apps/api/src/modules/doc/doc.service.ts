...
export class DocService {
  ...
  async generatePDF(prompt: string, sessionId: string): Promise<GenerateDocResult> {
    ...
    // 2. Envolver em HTML completo com estilos
    const fullHtml = `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body>${html}</body>
  ...
  async generateDOCX(prompt: string, data: Record<string, unknown>, sessionId: string): Promise<GenerateDocResult> {
    // Sem template customizado — gera DOCX simples com conteúdo do LLM
    const res = await this.llm.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        {
...
Língua: português brasileiro.`,
    ...
    // Template DOCX mínimo com placeholder {content}
    ...
    if (fs.existsSync(templatePath)) {
      ...
      // Sem template: salva como .txt com extensão .docx (fallback simples)
